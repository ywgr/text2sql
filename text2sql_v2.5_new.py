...
# -*- coding: utf-8 -*-
"""
TEXT2SQL V2.5 - 基于copy.py内核 + V2.4 UI
完全使用copy.py的查询逻辑，保持V2.4的UI风格
"""
...
class VannaWrapper:
    ...
    def generate_sql(self, prompt):
        try:
            self.api_calls += 1
            print(f"🔍 正在调用DeepSeek API (第{self.api_calls}次调用)")
...
def main():
    ...
    system = st.session_state.system_v25_new
    ...
    if page == "SQL查询":
        ...
        show_system_monitoring_page(system)
...
def show_sql_query_page(system):
    ...
    with col1:
        ...
        # 初始化session state
        if 'current_sql_new' not in st.session_state:
            st.session_state.current_sql_new = ""
        if 'current_question_new' not in st.session_state:
            st.session_state.current_question_new = ""
        if 'current_db_config_new' not in st.session_state:
            st.session_state.current_db_config_new = None
        if 'query_results_new' not in st.session_state:
            st.session_state.query_results_new = None
        ...
        if st.button("生成SQL查询 (V2.5增强)", type="primary"):
            if question:
                with st.spinner("正在使用V2.5增强引擎生成SQL..."):
                    ...
                    if sql:
                        # 保存到session state
                        st.session_state.current_sql_new = sql
                        st.session_state.current_question_new = question
                        st.session_state.current_db_config_new = db_config
                        ...
                        with st.spinner("正在执行查询..."):
                            ...
                            if success:
                                ...
                                st.session_state.query_results_new = {
                                    'success': True,
                                    'df': df,
    ...
    with col2:
        ...
        if st.session_state.query_results_new:
            results = st.session_state.query_results_new
...
def show_system_monitoring_page(system):
    """系统监控页面"""
    st.header("系统监控 V2.5")
    st.subheader("系统状态")
    ...
    if historical_count > 0:
        recent_qa = system.historical_qa[-5:]
        for qa in recent_qa:
