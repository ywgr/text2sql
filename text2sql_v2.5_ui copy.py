            ...
            if question:
                with st.spinner("正在生成SQL..."):
                    ...
                    with st.expander("显示详细LLM过程", expanded=False):
                        st.subheader("Prompt 详情")
                        st.code(prompt, language="text")
                        st.subheader("LLM 分析过程")
                        st.write(analysis)
                    ...
                    if sql:
                        ...
                        if not field_validation['all_valid']:
                            st.error(f"字段验证失败：以下字段不存在于表结构中：{', '.join(field_validation['missing_fields'])}")
                            st.info("建议：请检查表结构知识库，确保所有字段都已正确导入")
